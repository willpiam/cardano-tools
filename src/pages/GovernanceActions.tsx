...
const GovernanceActions = () => {
  ...
  const setActionByKey = (
    ...
    setActions((prev) =>
      prev.map((action) => (action.txHash === txHash && action.certIndex === certIndex ? updater(action) : action))
    );
  };
  ...
  const fetchData = async (key: string) => {
    ...
    try {
      const loaded = await fetchLiveGovernanceActions(key, {
        onPartial: (partial) => setActions(partial),
      });
      setActions(loaded);
    } catch (err) {
  ...
  const filteredActions = useMemo(() => {
    const base =
      selectedType === 'all' ? [...actions] : actions.filter((action) => action.governanceType === selectedType);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      >
        ...
        >
          ...
          {!loading && !error && actions.length > 0 && (
            <>
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                <span>
                  Total live actions: <strong>{actions.length}</strong>
                </span>
                {GOVERNANCE_TYPES.map((type) => (
                  <span key={type}>
                ...
                {filteredActions.map((action) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {displayTitle && (
                        <div style={{ color: '#f8fafc', fontWeight: 700, fontSize: '0.95rem' }}>{displayTitle}</div>
                      )}
                      ...
                      <div style={{ fontSize: '0.8rem', color: '#cbd5e1', display: 'grid', gap: '0.25rem' }}>
                        <div>
                          ...
                          <strong>
                            {action.metadataStep1Status === 'success'
                              ? 'success'
                              : action.metadataStep1Status === 'error'
                                ? 'error'
                                : 'loading'}
                          </strong>
                        </div>
                        ...
                        {action.metadataHash && (
                          <div style={{ wordBreak: 'break-all' }}>Hash: {action.metadataHash}</div>
                        )}
                        <div>
                          Metadata document fetch: <strong>{action.metadataStep2Status}</strong>
                        </div>
                      </div>
                      ...
                      {action.metadataError && (
                        ...
                        >
                          <div>
                            {action.metadataError.source === 'step1' ? 'Step 1' : 'Step 2'}:{' '}
                            {action.metadataError.message}
                          </div>
                          ...
                          {action.metadataStep2Status === 'error' && action.metadataUrl && (
                            <div>
                              <Button onClick={() => runMetadataStep2(action.txHash, action.certIndex)}>
                                Retry metadata
                              </Button>
                            </div>
                          )}
                      ...
                      {(action.ratifiedEpoch !== null || action.enactedEpoch !== null) && (
                        <div style={{ fontSize: '0.8rem', color: '#cbd5e1' }}>
                          {action.ratifiedEpoch !== null && (
                            <span style={{ marginRight: '0.75rem' }}>Ratified epoch: {action.ratifiedEpoch}</span>
                          )}
          ...
          {!loading && !error && apiKey && actions.length === 0 && <p>No live governance actions found.</p>}
        </div>
      </div>
    </div>
  );
