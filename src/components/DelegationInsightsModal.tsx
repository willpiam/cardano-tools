...
export function DelegationInsightsModal({
  open,
  onClose,
  title,
  titleId,
  ...
  controls,
  footnote,
}: DelegationInsightsModalProps) {
  const [visibility, setVisibility] = useState<DelegationInsightsVisibility>(
    DEFAULT_DELEGATION_INSIGHTS_VISIBILITY
  );

  const bucketMeta = useMemo(() => delegationBucketMeta(featuredSetLabel), [featuredSetLabel]);
  ...
  useEffect(() => {
    if (open) {
      setVisibility(DEFAULT_DELEGATION_INSIGHTS_VISIBILITY);
    }
  }, [open]);
  ...
  const chartData = useMemo(() => {
    ...
    return chartSlicesFromCounts(bucketMeta, counts);
  }, [breakdown, visibility, bucketMeta]);
  ...
  const modal = (
    <div className="ipfs-link-modal-overlay" role="presentation" onClick={onClose}>
      ...
      >
        ...
        >
          <h2 id={titleId} className="ipfs-link-modal-title">
            {title}
          </h2>
        ...
        {!loading && !error && breakdown && (
          <>
            ...
            <p style={{ margin: footnote ? '0.75rem 0 0' : 0, fontSize: '0.85rem', color: '#9ca3af', textAlign: 'center' }}>
