...
export function GovernanceActionMetadataModal({
  ...
  const jsonText =
    rawPayload !== null ? JSON.stringify(rawPayload, null, 2) : null;

  const modal = (
    <div className="ipfs-link-modal-overlay" role="presentation" onClick={status !== 'loading' ? onClose : undefined}>
      <div
        className={panelClass}
        role="dialog"
        aria-labelledby="governance-metadata-title"
        aria-modal="true"
        aria-busy={status === 'loading'}
        onClick={(e) => e.stopPropagation()}
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
          <h2 id="governance-metadata-title" className="ipfs-link-modal-title">
            Governance metadata
          </h2>
          <button
            type="button"
        ...
        <p className="ipfs-link-modal-muted governance-metadata-body" style={{ marginBottom: '1rem' }}>
          Action:{' '}
          <code className="ipfs-link-modal-code" style={{ display: 'inline', padding: '0.15rem 0.35rem', margin: 0 }}>
            {proposalLabel}
          </code>
        </p>
        ...
        {status === 'loading' && (
          <>
            ...
            <p className="reloading-recache-description">Loading metadata…</p>
          </>
        )}
        ...
        {status === 'loaded' && metadata && (
          <>
            ...
            {contentView === 'formatted' ? (
              <GovernanceMetadataView metadata={metadata} />
            ) : (
        ...
        {status === 'error' && error && (
          ...
          >
            ...
            <div className="governance-metadata-body" style={{ fontSize: '0.78rem', color: '#fca5a5' }}>
              Code: {error.code}
              {error.statusCode !== undefined ? ` · HTTP ${error.statusCode}` : ''}
              {error.details ? ` · ${error.details}` : ''}
            </div>
            {lastFetchUrl && (
              <div className="governance-metadata-body" style={{ fontSize: '0.75rem', color: '#d1d5db' }}>
            ...
            {isIpfsAnchor && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center' }}>
                <label htmlFor="metadata-gateway-select" style={{ fontSize: '0.85rem', color: '#d1d5db' }}>
                  Gateway:
                </label>
                <select
                  id="metadata-gateway-select"
                ...
                <button
                  type="button"
                  className="ipfs-link-modal-copy-btn"
                  onClick={handleRetry}
                >
                  Try again
                </button>
